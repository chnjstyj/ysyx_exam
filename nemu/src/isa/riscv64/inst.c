...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 0011011",slliw,I,R(dest) = SEXT(BITS(src1 << BITS(imm,4,0),31,0),32));
  INSTPAT("0000000 ????? ????? 101 ????? 0011011",srliw,I,R(dest) = SEXT(BITS(src1 >> BITS(imm,4,0),31,0),32));
  INSTPAT("0100000 ????? ????? 101 ????? 0011011",sraiw,I,R(dest) = SEXT(BITS((long)src1 >> BITS(imm,4,0),31,0),32));
