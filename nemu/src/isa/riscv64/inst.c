...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 0010011",srai,I,R(dest) = (long)src1 >> BITS(imm,5,0));
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 0111011",addw,R,R(dest) = SEXT(BITS(src1 + src2,31,0),31));
  INSTPAT("0100000 ????? ????? 000 ????? 0111011",subw,R,R(dest) = SEXT(BITS(src1 - src2,31,0),31));
  INSTPAT("0000000 ????? ????? 001 ????? 0111011",sllw,R,R(dest) = SEXT(BITS(src1 << BITS(src2,4,0),31,0),31));
  INSTPAT("0000000 ????? ????? 101 ????? 0111011",srlw,R,R(dest) = SEXT(BITS(src1 >> BITS(src2,4,0),31,0),31));
  INSTPAT("0100000 ????? ????? 101 ????? 0111011",sraw,R,R(dest) = SEXT(BITS((long)src1 >> BITS(src2,4,0),31,0),31));
  INSTPAT("???????????? ????? 110 ????? 0000011",lwu,I,R(dest) = (uint32_t)BITS(Mr(imm+src1,4),31,0));//零拓展，若为符号扩展，则将其先转换成同位宽的有符号数
  INSTPAT("???????????? ????? 010 ????? 0000011",lw,I,R(dest) = (int32_t)BITS(Mr(imm+src1,4),31,0));
  INSTPAT("???????????? ????? 100 ????? 0000011",lbu,I,R(dest) = (uint16_t)BITS(Mr(imm+src1,2),15,0));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 1100011",bne,B,s->dnpc = src1 != src2 ? imm + s->pc : s->snpc;printf("imm:%lx\n",imm));
