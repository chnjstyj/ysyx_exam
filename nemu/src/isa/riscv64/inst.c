...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("???????????? ????? 000 ????? 0011011",addiw,I,R(dest) = SEXT(imm + src1,64));
  INSTPAT("???????????? ????? 010 ????? 0010011",slti,I,R(dest) = src1 < imm ? 1:0);
  INSTPAT("???????????? ????? 011 ????? 0010011",sltiu,I,R(dest) = src1 == 0 ? 1:0);
  ...
  INSTPAT("010000 ?????? ????? 001 ????? 0010011",srai,I,R(dest) = (long)src1 >> BITS(imm,5,0));
  INSTPAT("0000000 ????? ????? 001 ????? 0011011",slliw,I,R(dest) = SEXT(BITS(src1 << BITS(imm,4,0),31,0),64));
  INSTPAT("0000000 ????? ????? 101 ????? 0011011",srliw,I,R(dest) = SEXT(BITS(src1 >> BITS(imm,4,0),31,0),64));
  INSTPAT("0100000 ????? ????? 101 ????? 0011011",sraiw,I,R(dest) = SEXT(BITS((long)src1 >> BITS(imm,4,0),31,0),64));
