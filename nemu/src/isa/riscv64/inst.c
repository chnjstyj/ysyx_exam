...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 0110011",rem,R,R(dest) = (int64_t)src1 % (int64_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 0110011",divu,R,R(dest) = src1 / src2);
  INSTPAT("0000001 ????? ????? 100 ????? 0110011",div,R,R(dest) = (int64_t)src1 / (int64_t)src2);

  INSTPAT("???????????? ????? 001 ????? 1110011",csrrw,I,csr(imm) = src1;if (dest != 0) {R(dest) = csr(imm);csr(imm) = src1;});
  INSTPAT("???????????? ????? 010 ????? 1110011",csrrs,I,R(dest) = csr(imm);if (src1 != 0) {csr(imm) = (csr(imm) | (uint64_t)(1 << src1));});
  ...
  INSTPAT("0011000 00010 00000 000 00000 1110011", mret , I, s->dnpc = csr(mepc),csr(mstatus) = 0xa00000080); 
