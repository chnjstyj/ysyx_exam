...
class cache_controller(
    ...
    val addr_offset = WireDefault(cache_addr(offset_width-1,0))
    ...
    }.elsewhen (next_state === s3 && cur_state === s1){
        cache_writeback_index := cache.io.writeback_index
    }

    when (cur_state === s2){
        io.mem_addr := cache_writeback_addr
    ...
    }.elsewhen (cur_state === s1 && !crossline_access){
