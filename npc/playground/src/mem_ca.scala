...
class mem_ca extends Module{
    val io = IO(new Bundle{
        ...
        val mem_csr_wen = Input(UInt(1.W))
        val mem_csr_addr = Input(UInt(12.W))
        val mem_csr_write_to_reg = Input(UInt(1.W))
        val mem_csr_rdata = Input(UInt(64.W))
    ...
    io.ca_csr_wen := RegEnable(io.mem_csr_wen,0.U,enable)
    io.ca_csr_addr := RegEnable(io.mem_csr_addr,0.U,enable)
    io.ca_csr_write_to_reg := RegEnable(io.mem_csr_write_to_reg,0.U,enable)
    io.ca_csr_rdata := RegEnable(io.mem_csr_rdata,0.U,enable)
    ...
    io.ca_ecall := RegEnable(io.mem_ecall,0.U,enable)
